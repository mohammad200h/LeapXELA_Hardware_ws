...
def _has_uniform_scale(
    ...
    """True when both sides of zero use 1:1 sim-to-hardware scaling."""
...
def _uniform_offset_map(value: float, hw_zero: float, inverted: bool) -> float:
    """
    Map a 0-centered sim angle to hardware with 1:1 scale.

    Inverted hardware limits flip sign; otherwise add.
    """
    if inverted:
...
def map_sim_to_hardware(
    ...
    """
    Map sim joint angles to hardware encoder angles.

    Sim joints use a 0-centered frame (zero ≈ 0, lower < upper). Hardware
    joints use encoder zeros; many have inverted limits (lower > upper in
    joint_config.json).

    Most joints are a constant offset with optional sign flip:
      - inverted hardware axis: hw_zero - sim
      - same direction:         hw_zero + sim

    Joints with mismatched segment scaling (e.g. thumb axl, thumb mcp) use a
    ...
    for i, v in enumerate(sim_value):
        ...
        if abs(sim_zero) < 1e-9 and _has_uniform_scale(
            ...
            out.append(_uniform_offset_map(v, hw_zero, hw_ll > hw_ul))
        else:
            out.append(
