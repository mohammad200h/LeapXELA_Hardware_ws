...
def _load_workspace_joint_config() -> dict:
    ...
    raise FileNotFoundError("Could not find xela_description/joint_config.json in workspace parents")
...
def test_get_joint_names():
    ...
    hardware_joint_names = get_ordered_joint_state_message_with_index_as_joint_value().name
...
def test_get_joint_ranges():
    ...
    from samples import get_ordered_joint_state_message_with_index_as_joint_value, get_joint_ranges_hw, get_joint_ranges_sim

    joint_config = _load_workspace_joint_config()
    ordered_joint_names = get_ordered_joint_state_message_with_index_as_joint_value().name
    # Hardware
    joint_ranges_hw = get_joint_ranges(ordered_joint_names, joint_config["leapXela"]["hardware"])
    ...
    # Sim
    joint_ranges_sim = get_joint_ranges(ordered_joint_names, joint_config["leapXela"]["sim"])
...
def test_remap_joint_commands():
    ...
    unordered_joint_state_message = get_unordered_joint_state_message_with_index_as_joint_value()
    ordered_joint_state_message = get_ordered_joint_state_message_with_index_as_joint_value()
...
def test_map_sim_to_hardware():
    from samples import get_joint_ranges_hw, get_joint_ranges_sim, get_hardware_joints_at_zero_position
    ...
    # test conversion at lower limits
    sim_values = joint_ranges_sim["ll"]
    hardware_ll = joint_ranges_hw["ll"]
    hardware_values = map_sim_to_hardware(sim_values, joint_ranges_sim, joint_ranges_hw)
    assert hardware_values == pytest.approx(hardware_ll, rel=1e-5, abs=1e-2) , "mapping at lower limits failed"
    

    # test conversion at upper limits
    sim_values = joint_ranges_sim["ul"]
    hardware_ul = joint_ranges_hw["ul"]
    hardware_values = map_sim_to_hardware(sim_values, joint_ranges_sim, joint_ranges_hw)
    assert hardware_values == pytest.approx(hardware_ul, rel=1e-5, abs=1e-2) , "mapping at upper limits failed"

    # test conversion at zero position
    sim_values = [0.0]*16
    hardware_values = map_sim_to_hardware(sim_values, joint_ranges_sim, joint_ranges_hw)
    assert hardware_values == pytest.approx(hardware_joints_at_zero_position, rel=1e-5, abs=1e-2) , "mapping at zero position failed"
...
def _test_convert_sim_to_hardware_publishes_ordered_and_mapped(monkeypatch):
    ...
    try:
        ...
        pub = helper.create_publisher(JointState, "oculus_teleop_joint_commands", 10)
        ...
        while time.time() < deadline and "msg" not in received:
            # helper published unordered joint state message
            pub.publish(unordered_joint_state_message)
            ex.spin_once(timeout_sec=0.05)

        assert "msg" in received, "Timed out waiting for converted JointState on cmd_xela"
...
# def test_convertion_at_lower_limits(monkeypatch):
#     from oculusTeleop import sim_to_hardware_conversion as conv
