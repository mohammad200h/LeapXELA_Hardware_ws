...
def _nano17_rows(nano_grp: Any) -> List[List[Any]]:
    """One row per axis from HDF5 `nano17` group (matches Storage._write_wrench)."""
    force = np.asarray(nano_grp["force"][()]) if "force" in nano_grp else np.zeros(3, dtype=np.float64)
    torque = np.asarray(nano_grp["torque"][()]) if "torque" in nano_grp else np.zeros(3, dtype=np.float64)
    rows: List[List[Any]] = []
    for axis, i in zip(("x", "y", "z"), range(3)):
        rows.append(
            [
                axis,
                float(force[i]) if i < len(force) else None,
                float(torque[i]) if i < len(torque) else None,
            ]
        )
    return rows
...
def _load_sample(
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        nano17_rows: List[List[Any]] = []
        if "nano17" in grp:
            nano17_rows = _nano17_rows(grp["nano17"])
        ...
        meta: Dict[str, Any] = {
            ...
            "nano17_group_attrs": (
                {k: _jsonable(grp["nano17"].attrs.get(k)) for k in grp["nano17"].attrs.keys()}
                if "nano17" in grp
                else {}
            ),
            "nano17_present": "nano17" in grp,
        }
    ...
    return raw_x_rows, raw_y_rows, raw_z_rows, ch0, ch1, ch2, rows, xela_rows, xela_xyz_rows, nano17_rows, meta
...
def visulize_dataset() -> None:
    ...
    with gr.Blocks(title="XELA + NANO17 dataset visualizer") as demo:
        gr.Markdown("## XELA + NANO17 dataset visualizer")
        ...
        gr.Markdown("### nano17 force/torque")
        nano17_out = gr.Dataframe(
            label="nano17 wrench (force / torque)",
            value=[["", None, None]],
            headers=["axis", "force", "torque"],
        ...
        def update(dataset_file: str, h5_path: str, stream_name: str, index: int):
            ...
            r0, r1, r2, ch0, ch1, ch2, rows, xela_rows, xela_xyz_rows, nano17_rows, meta = _load_sample(
            ...
            if not nano17_rows:
                nano17_rows = [["", None, None]]
            return r0, r1, r2, ch0, ch1, ch2, rows, xela_rows, xela_xyz_rows, nano17_rows, meta
