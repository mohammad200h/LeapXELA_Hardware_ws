...
CallbackReturn FrankaMultiHardwareInterface::on_activate(
  ...
  for(auto& arm_container_pair : arms_){
    ...
    arm.robot_->setControlMode(ControlMode::None);
...
hardware_interface::return_type FrankaMultiHardwareInterface::perform_command_mode_switch(
  ...
  for(auto& arm_container_pair : arms_){
    ...
    if(arm.switch_cm_){
      arm.robot_->setModeSwitchInProgress(true);
      ...
      arm.robot_->setControlMode(arm.control_mode_);
      arm.robot_->setModeSwitchInProgress(false);
