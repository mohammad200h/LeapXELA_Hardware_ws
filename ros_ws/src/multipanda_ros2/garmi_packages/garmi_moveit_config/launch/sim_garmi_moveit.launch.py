...
def generate_launch_description():
    ...
    garmi_bringup_path = get_package_share_directory('garmi_bringup')
    ...
    moveit_simple_controllers_yaml = load_yaml(
        'garmi_moveit_config', 'config/sim_garmi_controllers.yaml'
    )
    moveit_controllers = {
    ...
    rviz_full_config = os.path.join(rviz_base, 'garmi_moveit.rviz')
    ...
    ros2_controllers_path = os.path.join(
        get_package_share_directory('garmi_moveit_config'),
        'config',
        'sim_garmi_ros_controllers.yaml',
    )

    # Mujoco ros2 server
    mujoco_ros2_node = IncludeLaunchDescription(
            FrontendLaunchDescriptionSource(garmi_bringup_path + '/launch/sim/launch_mujoco_ros_server.launch'),
            launch_arguments={
                'use_sim_time': "true",
                'modelfile': xml_path,
                'verbose': "true",
    ...
    # Load controllers
    load_controllers = []
    for controller in ['garmi_arm_controller', 'joint_state_broadcaster']:
        load_controllers += [
            ExecuteProcess(
    ...
    if(load_gripper):
        jsp_source_list.append(concatenate_ns('', 'left_gripper_sim_node/joint_states/joint_states', True))
        jsp_source_list.append(concatenate_ns('', 'right_gripper_sim_node/joint_states/joint_states', True))
