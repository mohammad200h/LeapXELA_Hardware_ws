...
class LeapNode:
    def __init__(self):
        ...
        self.kD = 200
        self.curr_lim = 550
        # Thumb-only: control Dynamixel IDs 0..3 (4 motors total)
        self.motors = motors = [0, 1, 2, 3]
        self.prev_pos = self.pos = self.curr_pos = np.zeros(len(motors), dtype=float)
        try:
        ...
        # If you want some joints to have lower gains, list them here (must be subset of 0..4).
    ...
    def set_joints_degrees(self, degrees_array):
        """Set thumb motors (IDs 0..3) using degree values (length 4)."""
        if len(degrees_array) != len(self.motors):
            print(f"Error: Please provide exactly {len(self.motors)} values (one per thumb motor).")
            return
        try:
    ...
    def read_pos_degrees(self):
        """Read current thumb motor positions and return in degrees."""
...
def main():
    ...
    # Define the base thumb configuration (motors 0..3 only).
    # Keep all thumb motors at 180 initially.
    desired_thumb_degrees = [180.0, 180.0, 180.0, 180.0]
    
    print("=== LEAP Hand Sinusoidal Thumb Control ===")
    print(f"Frequency: {frequency} Hz")
    print(f"Number of cycles: {num_cycles}")
    print(f"Angle range: {min_angle}° to {max_angle}°")
    print(f"Starting position (motor 3): {max_angle}°")
    print("==========================================")
    
    # Set initial position
    leap_hand.set_joints_degrees(desired_thumb_degrees)
    ...
    try:
        ...
        while cycle_count < num_cycles:
            # Update thumb motor 3 angle (change this index if your phalange is a different ID).
            ...
            # Update the thumb configuration (motors 0..3 only)
            desired_thumb_degrees[3] = current_angle
