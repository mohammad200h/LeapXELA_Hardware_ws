...
class PositionNode(Node):
    def __init__(self):
        ...
        save_trajectory_artifacts = bool(self.get_parameter("save_trajectory_artifacts").value)
        trajectory_output_dir = str(self.get_parameter("trajectory_output_dir").value).strip()
        trajectory_csv_filename = str(self.get_parameter("trajectory_csv_filename").value).strip()
        trajectory_plot_filename = str(self.get_parameter("trajectory_plot_filename").value).strip()

        self.joint_names, self.joint_positions_deg = self.load_csv(csv_path, source_units=source_units)
...
def main(args=None):
    ...
    try:
        while rclpy.ok() and not position_node._shutdown_requested:
            rclpy.spin_once(position_node, timeout_sec=0.1)
    finally:
