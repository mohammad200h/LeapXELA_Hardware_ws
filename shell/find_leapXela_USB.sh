...
echo "Looking for FT232H USB-UART/FIFO devices (${TARGET_VENDOR_ID}:${TARGET_MODEL_ID})..."
...
if [ "$count" -ne "$EXPECTED_COUNT" ]; then
  echo "Expected $EXPECTED_COUNT matching device(s) (${TARGET_VENDOR_ID}:${TARGET_MODEL_ID}) but found $count."
  echo "Matches found:"
  if [ "$count" -eq 0 ]; then
    echo "  (none)"
  else
