#!/usr/bin/env python3
"""Interactive PyBullet viewer for the LeapXela right hand URDF.

Loads ``hand.urdf`` and creates one GUI slider per revolute joint, bounded by
that joint's lower/upper limits from the URDF. Fixed joints are ignored.
Moving a slider updates the corresponding finger joint in real time.
"""
...
def _slider_joints(body_id: int) -> list[tuple[int, str, float, float]]:
    ...
    Fixed joints (base attachment, realtip markers) are ignored — no sliders.
    """
...
def run_viewer(*, use_gui: bool = True) -> None:
    ...
    try:
        ...
        body_id = p.loadURDF(
            urdf_path,
            basePosition=[-0.12, 0.0, 0.01],
            baseOrientation=p.getQuaternionFromEuler([1.57, 0, -1.57]),
            useFixedBase=True,
        )
        if body_id < 0:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="PyBullet LeapXela hand viewer with per-joint sliders."
    )
    parser.add_argument(
        "--direct",
        action="store_true",
